...
from src.prompt_values.long_memory import LongTermMemoryPromptValue, LongTermMemoryExtractPromptValue
...
class LongTermMemoryUpdater(RunnableSerializable[dict[str, Any], str]):
    ...
    def _load_long_memory(self) -> str:
        ...
        try:
            ...
            raise ValueError(f"Failed to decode long-term memory JSON: {e}")
    ...
    def extract(
        ...
        if not isinstance(history, list):
            raise TypeError(
                f"Expected a list for {self.history_variable_key}, got {type(history)}"
            )
